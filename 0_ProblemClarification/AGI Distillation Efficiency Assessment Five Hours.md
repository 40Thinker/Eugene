...
|--------|-------------------|
| [[Overlay AGI Comprehensive System Development]] | Описывает полную архитектуру Overlay AGI: разделение на внешние знание‑базы, нейронные слои и символический рассудок, а также O(1)‑вычислительную эффективность за счёт предвычисленных семантических весов [^1]. |
| [[01_Framework]] | Формирует консенсус‑основу «идеального искусственного интеллекта»: философские критерии, архитектурные принципы, технические возможности и стандарты оценки [^2]. |
| [[08_AI_Architecture_Review_Framework]] | Предлагает методику анализа 50 ключевых компонентов ИИ (история, трансформеры, гибриды, мета‑обучение) – полезно при выборе элементов для Overlay‑системы [^4]. |
| [[14_Comprehensive_AI_Architecture_Review]] | Содержит готовый список из 50 архитектурных блоков с описанием сильных/слабых сторон и рейтинговой оценкой, который можно использовать как «каталог компонентов» при построении собственного стека [^3]. |

---

### 2️⃣ Прямо относящиеся к этой заметке  
*(конкретные идеи из текущего обсуждения Distillation‑Efficiency)*  

| Ссылка | Краткое пояснение |
|--------|-------------------|
| [[AGI Distillation Efficiency Assessment Five Hours]] | Текущий активный документ, в котором сравнивается «5 ч» локального дистилляционного проекта с типичными пользовательскими сценариями и раскрываются биологические метафоры (сперматозоид, ДНК‑упаковка) [^13]. |
| [[AGI Replication via Architectural Seed]] | Поясняет, почему AGI нельзя «скопировать», а нужно выращивать из архитектурного семени — ключ к пониманию репликации и «передачи» интеллекта [^11]. |
| [[Limits of Overlay AGI in LLM Architectures]] | Выделяет границы Overlay‑подхода: без человеческого участия эффективность резко падает, что важно учитывать при планировании автономных модулей [^12]. |

---

### 3️⃣ Нижестоящие идеи  
*(детализированные подпункты, которые помогут реализовать отдельные части системы)*  

| Ссылка | Краткое пояснение |
|--------|-------------------|
| [[02_Philosophical_Criteria]] | Десять философских требований (когнитивная целостность, метакогнитивное осознание, моральный рассудок и др.) – базис для построения «самосознающего» AGI [^5]. |
| [[03_Architectural_Principles]] | Десять архитектурных принципов (модульная совместимость, масштабируемость, динамическое распределение ресурсов…) — практический набор правил для проектирования гибкой Overlay‑системы [^6]. |
| [[04_Technical_Capabilities]] | Перечисление технических способностей (реальное‑время, быстрый рост знаний, кросс‑доменно­й перенос и т.д.) — критерий при выборе конкретных моделей и инфраструктуры [^7]. |
| [[05_Practical_Excellence]] | Десять критериев практической исключительности (человеческая совместимость, надёжность, безопасность) — полезно для UI/UX‑дизайна и оценки готовности продукта к эксплуатации [^8]. |
| [[12_AI_Architecture_Components_Part2]] | Оценка 30‑35‑их компонентов (Multi‑Task Learning, Contrastive Learning, Sparsity Optimization, Quantization) — детали реализации оптимизаций, которые могут быть включены в RAG‑слой или в семантические весовые таблицы [^9]. |
| [[13_AI_Architecture_Components_Part3]] | Оценка 41‑50‑их компонентов (Continuous Learning, Distributed Memory, Neurosymbolic Integration, Dynamic Routing) — идеи для построения «само‑эволюционирующего» ядра Overlay‑AGI [^10]. |
