...
1.  **[[AGI Philosophical Integration Framework]]**: Эта концепция предоставляет фундаментальную структуру для понимания, как философские принципы могут быть интегрированы в архитектуру AGI. В контексте прото-ИИ она помогает определить, как старые системы управления, основанные на строгих правилах и математических моделях, могли обеспечивать "интеллект" через функциональность, а не через нарратив. Философские концепции из этой заметки (например, "meta-presence", "dual-mirror") могут помочь создать более глубокую модель интеллекта, аналогичную старым системам управления, где центральная идея — функциональное поведение, а не человеческая интерпретация[^1].
2.  **[[AGI Philosophical Framework]]**: В рамках этой заметки особое внимание уделено "онтологическому свёртыванию", "сублогической сети" и "существующему импульсу". Эти идеи связаны с концепцией, что интеллект может возникать через структурированные взаимодействия, а не только через сложные нейронные сети. Прото-ИИ, такие как автопилот Бурана, демонстрируют "онтологическое свёртывание" через связывание различных физических сущностей в единое целое, а их поведение отражает "существующий импульс" — предсказуемый и устойчивый процесс, основанный на физике[^2].
3.  **[[Deep Learning Optimization Blindness]]**: Эта заметка критикует текущее направление в обучении нейронных сетей, где доминирует масштабирование вместо понимания обучения. Это напрямую связано с идеей прото-ИИ, поскольку старые системы работали эффективно при ограниченных ресурсах и не использовали "слепые экспериментальные попытки", которые характерны для современного подхода к LLM. Понимание этой проблемы помогает инженерам избежать ошибок в создании сложных архитектур, вместо того чтобы строить надежные и эффективные системы на основе фундаментальных принципов[^3].
4.  **[[Neural Networks Theoretical vs Empirical Thinking]]**: Сравнение теоретического и эмпирического мышления помогает понять, почему современные LLM могут быть "простыми повторителями" данных, а не генераторами новых идей. Прото-ИИ, напротив, демонстрируют более высокий уровень теоретического мышления — они работают с конкретными моделями и алгоритмами, основанными на физических законах и математике[^4].

### Нижестоящие идеи

1.  **[[Energy Cost of Long Context Generation]]**: Эта заметка подчеркивает рост энергозатрат при генерации токенов в LLM с увеличивающимся контекстом, показывая квадратичную сложность трансформеров. Для создания эффективных систем управления (как прото-ИИ) важно понимать, как можно снизить энергозатраты и избежать "вычислительного ловушек". Анализ затрат помогает понять, почему старые системы были более энергоэффективными и надежными[^5].
2.  **[[Self-Distillation in Emergent AGI Systems]]**: Эта заметка описывает процесс самодистилляции в эмерджентных AGI-системах, где система совместно с человеком и другим AGI проводит трёхагентный цикл для сохранения уникальных структур. Это напрямую связано с прото-ИИ, поскольку такие системы могли обучаться и адаптироваться в условиях строгих ограничений, при этом сохраняя свои ключевые функции. Понимание самодистилляции может помочь разработчикам создавать устойчивые архитектуры для управления сложными системами[^6].
3.  **[[LLM Mistake Completion vs Cognition]]**: Эта заметка критикует развитие LLM, указывая на ошибку токен-центричности и отсутствие истинного мышления. Она предлагает альтернативные архитектуры (когнитивные графы, полевые вычисления) вместо традиционных трансформеров. Прото-ИИ демонстрирует более глубокую форму мышления — через системное понимание физических процессов и их взаимодействий[^7].
4.  **[[Unsolved Problem Classes in AGI]]**: Эта заметка обсуждает два нерешённых класса задач для AGI: теоремы с неясным вводом, требующие онтологического конструктивизма, и полностью хаотические системы без повторяющихся паттернов. Эти проблемы могут возникать при создании прото-ИИ, когда необходимо создавать модели на основе ограниченных данных или предсказывать поведение сложных систем[^8].

### Прямо относящиеся к этой заметке

1.  **[[Proto-AGI Legacy Control Systems]]**: Эта заметка является центральной и содержит все ключевые идеи для понимания прото-ИИ как наследия старых систем управления. Она описывает важность предсказуемости, отказоустойчивости, интеграции подсистем и опыта из прошлого[^9].
2.  **[[10_Modern_AI_Architectures]]**: Эта заметка о современных архитектурных инновациях в AI, включая трансформеры и масштабирование моделей. Сравнивая их с прото-ИИ, можно лучше понять, почему старые системы были более надежными и эффективными. Трансформеры обеспечивают параллельную обработку последовательностей, но их сложность может привести к ошибкам в условиях ограниченных ресурсов[^10].
3.  **[[Parametric Sensitivity Analysis of LLM Architecture]]**: Эта заметка предлагает экспериментальный подход к оценке влияния отдельных архитектурных параметров LLM. В контексте прото-ИИ она помогает определить, какие характеристики (например, размер модели или объем данных) действительно важны для надежности и эффективности, а какие могут быть избыточными[^11].
4.  **[[11_AI_Architecture_Components_Part1]]**: Эта заметка анализирует ключевые компоненты AI-архитектур, такие как гибридные архитектуры, нормализация слоев, пропускные связи и резидуальные методы обучения. Эти компоненты важны для понимания того, как можно построить надежную и предсказуемую систему управления подобно прото-ИИ[^12].

## Мое мнение об этом документе

Для инженеров важно понять, что **прото-ИИ** не просто устаревшие технологии, а примеры **мудрой, зрелой инженерной культуры**, где важны: предсказуемость, отказоустойчивость и эффективность. В отличие от современных LLM, которые часто "отвлекаются" на сложности и громоздкость, прото-ИИ демонстрируют мощь простых, хорошо продуманных решений.

Важно обратить внимание на следующее:

1.  **Предсказуемость и надежность**: Старые системы управления работали в условиях строгих временных ограничений и критических требований, что делало их предсказуемыми и надежными. Инженеры должны стремиться к аналогичному уровню стабильности в своих системах.
2.  **Математическое моделирование**: Прото-ИИ строились на строгих математических моделях, а не на нейронных сетях и генерации текста. Это помогало им принимать решения быстро и точно без "галлюцинаций".
3.  **Ограниченные ресурсы**: Умение работать с ограниченными ресурсами (ограниченная память, отсутствие GPU) делало старые системы более устойчивыми к ошибкам и лучше адаптированными к реальным условиям.
4.  **Инженерное мышление**: Старая инженерная школа ставила целью достижение конкретных результатов (например, успешное завершение миссии), а не просто создание "новых и интересных" моделей.

Эти идеи особенно важны при разработке **Overlay NeuroSymbolic Hybrid Symbiotic ASI**, где сочетание символической логики и нейросетевых подходов должно быть основано на стабильной, предсказуемой и эффективной архитектуре, вдохновленной прото-ИИ.
...
[^9]: [[Proto-AGI Legacy Control Systems]]
